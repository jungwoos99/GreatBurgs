...
export default function MenuFoodCard(props) {
    ...
    function addRemoveItem() {
        ...
        if(!isAdded) {
            Cookies.set(`cartTotal${userId}`, (previousCartTotal + price))
            dispatch(increaseCartTotal(Number.prototype.valueOf(price)))
            dispatch(increaseCartQuantity())
            setIsAdded(true)
            if(Cookies.get(`cart${userId}`)) {
                const cookieCart = Cookies.get(`cart${userId}`)
                Cookies.set(`cart${userId}`, cookieCart + (id + ","))
            } else {
                Cookies.set(`cart${userId}`, (id+ ","))
            }
        } else {
            if(cartTotal >= price) {
                dispatch(decreaseCartTotal(Number.prototype.valueOf(price)))
